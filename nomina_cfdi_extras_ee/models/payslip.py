...
#                     result[line.code] = line.name
    ...
    def export_report_xlsx(self):
        ...
        worksheet.write(0, 3, 'Categ.', header_style)
        worksheet.write(0, 4, 'Antiguedad', header_style)
        col_nm = 5
        ...
        for dept in self.env['hr.department'].browse(payslip_group_by_department.keys()).sorted(lambda x:x.name):
            ...
            for slip in hr_payslips:
                ...
                worksheet.write(row, 3, slip.employee_id.contract_id[0].categoria_lucerna, text_right)
                worksheet.write(row, 4, slip.employee_id.contract_id[0].date_start, text_right)
                code_col = 5
